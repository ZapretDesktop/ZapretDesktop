...
<img width="642" height="512" alt="image" src="https://github.com/user-attachments/assets/74f2f3e7-45cf-43fe-8370-7272038a108b" />
...
2. Убедитесь, что рядом с `ZapretDesktop.exe` есть папка **winws** (с `bin\winws.exe`, списками и `.bat`-стратегиями). Если её нет — при запуске откроется диалог настройки winws: можно **скачать** (через GitHub), **выбрать** папку или zip-архив, либо **пропустить**.
...
### Настройка папки winws

Если папка **winws** не найдена (рядом с exe или по пути из настроек), при старте открывается **диалог настройки winws**:

- **Скачать** — загрузка и установка zapret из GitHub (по умолчанию Flowseal zapret-discord-youtube).
- **Выбрать** — указать папку winws или локальный ZIP-архив.
- **Пропустить** — запустить приложение без winws (стратегии будут недоступны до добавления папки).

Программа отслеживает появление и изменение папки winws — список стратегий обновляется автоматически.

### Настройки приложения

Все настройки задаются в **Настройки** (Ctrl+,): язык, трей, автозапуск, путь к winws, репозиторий zapret и др.
...
- В списке отображается: имя стратегии, версия zapret (из service.bat), при внешнем запуске winws — **pid=** (идентификатор процесса).
- Заголовок окна: **ZapretDesktop —** *текущая стратегия* (обновляется при запуске и во время тестирования).
...
- **Режимы тестирования** — стандартные тесты (HTTP/TLS/Ping) и проверки DPI (TCP-порты 16–20, тест «заморозки»).
...
- **Пауза** — кнопка «Пауза»/«Продолжить» для приостановки тестов.
- **Автообновление главного окна** — во время тестирования список стратегий и заголовок главного окна автоматически показывают текущую тестируемую стратегию.
- Перед запуском тестов, если winws уже запущен, программа предлагает его остановить.
...
- Обновление выполняется **по файлам и подпапкам**, без полного удаления winws. В настройках можно задать список **игнорируемых подпапок winws** (по умолчанию игнорируется `lists`) — такие каталоги и их содержимое не изменяются при обновлении (сохраняются пользовательские файлы/правки).
...
| **Обновление** | Удалять проверку обновлений zapret из стратегий; поле «Репозиторий zapret» (GitHub owner/repo или полная ссылка); таблица **игнорируемых подпапок winws** при обновлении (по умолчанию `lists`, можно добавить `bin` или свои каталоги). |
...
### 12.1. Технические детали работы

- **Контроль winws.exe**:
  - Запуск стратегий происходит только для `.bat` из папки winws (имя и путь проверяются перед запуском).
  - В главном окне таймер раз в секунду через psutil ищет процесс `winws.exe`, отображает PID и текущую стратегию и может автоматически перезапустить стратегию при внезапном завершении (если включён автоперезапуск).
- **Автоопределение папки winws**:
  - Путь берётся из настроек (`winws_path`), иначе ищется `winws` рядом с exe/скриптом, иначе используется путь, выбранный в мастере первой настройки.
  - QFileSystemWatcher следит за базовой директорией и за самой папкой winws; при изменениях автоматически пересобирается список стратегий.
- **Обновление winws и игнорируемые подпапки**:
  - ZapretUpdater распаковывает архив во временную папку, находит источник winws и по одному обновляет файлы/папки в текущем winws (без полного удаления).
  - Список `update_ignore_folders` в конфиге (по умолчанию `["lists"]`) задаёт подпапки winws, которые при обновлении полностью пропускаются (файлы в них не удаляются и не перезаписываются).
...
Проверьте, что папка **winws** лежит рядом с exe (или путь к ней указан в настройках) и в ней есть **.bat**-файлы (кроме service.bat). При появлении или изменении папки winws список стратегий обновляется автоматически.
